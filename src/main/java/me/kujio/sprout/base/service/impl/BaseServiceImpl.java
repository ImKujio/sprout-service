...
import me.kujio.sprout.base.mapper.BaseMapperHolder;
...
public abstract class BaseServiceImpl<T extends BaseEntity> implements BaseService<T> {
    ...
    public BaseServiceImpl(EntityHandle<T> entityHandle) {
    ...
    public List<T> list(@NonNull Query query) {
        return BaseMapperHolder.getBaseMapper().list(
    ...
    public T get(Integer id) {
        Map<String, Object> row = BaseMapperHolder.getBaseMapper().get(entityHandle.getTable(), id);
    ...
    public Map<Integer, Map<String, Object>> all(Set<String> fieldNames) {
        ...
        return CacheUtils.getOrPut(cacheKey, () -> BaseMapperHolder.getBaseMapper().all(entityHandle.getTable(), fields));
    }

    @Override
    public boolean exist(Integer id) {
        return BaseMapperHolder.getBaseMapper().exist(entityHandle.getTable(), id) > 0;
    }
    ...
    public int count(@NonNull Where where) {
        ...
        return CacheUtils.getOrPut(cacheKey, () -> BaseMapperHolder.getBaseMapper().count(entityHandle.getTable(), where));
    }

    @Override
    public void add(T entity) {
        AddRst addRst = new AddRst();
        BaseMapperHolder.getBaseMapper().add(entityHandle.getTable(), entityHandle.unmap(entity), addRst);
    ...
    public void set(T entity) {
        ...
        BaseMapperHolder.getBaseMapper().set(entityHandle.getTable(), entityHandle.unmap(entity), entity.getId());
        log.info("set: {}", entity);
        CacheUtils.delPrefix(entityHandle.entityName());
        for (OnUpdateHook onUpdateHook : onUpdateHooks) {
            onUpdateHook.run();
    ...
    public void del(@NonNull Where where) {
        ...
        BaseMapperHolder.getBaseMapper().del(entityHandle.getTable(), where);
        log.info("del: {}", where);
        CacheUtils.delPrefix(entityHandle.entityName());
        for (OnUpdateHook onUpdateHook : onUpdateHooks) {
            onUpdateHook.run();
